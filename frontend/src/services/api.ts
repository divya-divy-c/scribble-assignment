...
export const api = {
  ...
  submitGuess(code: string, participantId: string, text: string) {
    return request<{ isCorrect: boolean; guess: Guess; room: RoomSnapshot }>(
  ...
  updateDrawing(code: string, participantId: string, data: string) {
    return request<{ ok: boolean }>(`/rooms/${encodeURIComponent(code)}/drawing`, {
      method: "PUT",
